...
function accessId(){
    ...
    // 자바 스크립트는 문자열 비교시에도 비교연산자를 사용한다
    if(bgColor == "red"){
...
function accessName() {
    ...
    for(let i=0; i<hobbyList.length; i++){
        ...
        // * radio, checkbox 전용속성 
        // .checked : 해당요소가 체크 되어 있으면 true, 아니면 false 반환
...
function accessCss(){
    ...
    document.querySelector("#css-div").style.border="3px soild red";
...
function readValue(){
    ...
    if( input.value.trim().length>0){
        ...
        // 요소.scrollTop=요소.scrollHeight : 스클롤이 현재 높이만큼 
    ...
    // input 태그에 연결된 키가 눌러졌을때 엔터인 경우 를 검사하는 함수 
