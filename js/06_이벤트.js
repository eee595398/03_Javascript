...
//고전/표전 이벤트모델은 랜더링된 HTML 요소에 
// 이벤트에 관련된 동작을 부여하는 코드 

// -> 랜더링이 되지 않은 요소에는 관련 동작을 추가할수 없다 

//(문제 원인 : html 코드 해석 순서(위->아래))

// 해결방법 : html 요소가 먼저 랜더링 된 후 js 코드 수행 할 수 있도록 body 태크 맨 아래 script 태그를 넣다
...
// 위에 동작이 존재하는데 중복해서 적용(고전 이벤트 모델 문제점 해결 확인)
document.getElementById("test3").addEventListener("click", function() {
...
// input1에 작성한 값이 변경되었을때 입력된 값으로 배경색 변경
document.getElementById("input1").addEventListener("change",function(){
 
    // change이벤트
    // test 관련 input 태그는 
    // 입력된 값이 변할때를 change라고 하지 않고
...
document.getElementById("moveNaver").addEventListener("click",function(e){
    ...
    e.preventDefault(); // HTML 요소가 가지고 있는 기본 이벤트를 막음(제거_)
